...
function formatTime(minutes, seconds) {
  ...
  const formattedSecond = String(s).padStart(2, '0');
...
function countVowels(str) {
  ...
  const founder = str.match(/[aeiouy]/gi);
  return founder ? founder.length : 0;
}
...
function isPalindrome(str) {
  const refactorStr = str.toLowerCase().replace(/[^a-z]/g, '');
  const reversedStr = refactorStr.split('').reverse().join('');
  return refactorStr === reversedStr;
}
  ...
  return str
    ...
    .map((charact) => {
      if (charact === charact.toUpperCase()) {
        return charact.toLowerCase();
      }
      return charact.toUpperCase();
    })
    .join('');
}
...
function encodeToRot13(str) {
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const refactorElem = str.charCodeAt(i);
    if (refactorElem >= 65 && refactorElem <= 90) {
      result += String.fromCharCode(((refactorElem - 65 + 13) % 26) + 65);
    } else if (refactorElem >= 97 && refactorElem <= 122) {
      result += String.fromCharCode(((refactorElem - 97 + 13) % 26) + 97);
    } else {
      result += str.charAt(i);
    }
  }
